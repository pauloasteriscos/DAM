...
- Revisões de texto podem atualizar evidência, estado de implementação e ligações ao estado atual sem alterar a decisão aceite.
- Estados usados:
  - `Aceite — implementado`
  - `Aceite — parcialmente implementado`
  - `Aceite — implementação planeada`
...
- [ADR-008 — Desbloqueio local e imediato da progressão](./ADR-008-desbloqueio-local-imediato.md) — **Aceite — parcialmente implementado**
- [ADR-009 — Conteúdo orientado por dados com activityId estável e revisionId imutável](./ADR-009-conteudo-versionado-activity-revision.md) — **Aceite — parcialmente implementado**
- [ADR-010 — Progression Engine separado da UI](./ADR-010-progression-engine-separado-ui.md) — **Aceite — parcialmente implementado**

## Estado atual relevante

À data desta revisão, a Fase 4.7 já possui mapa/mission detail e runtimes tecnicamente ligados ao conteúdo oficial, mas dois gates continuam abertos e não devem ser documentados como concluídos:

```text
4.7B.2A
learningLanguageCode
→ percurso oficial correto
→ progresso independente por idioma

4.7C
runtime concluído
→ conclusão local durável
→ recálculo/desbloqueio
